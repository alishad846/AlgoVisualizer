...
function contains(text, keyword) {
  ...
  // Short keywords such as NGE, BFS, DFS, DP and BST
  // should match only as complete words.
  if (normalizedKeyword.length <= 3) {
    ...
    return wholeWordPattern.test(normalizedText);
  }
...
function detectAlgorithm(problem) {
  ...
  ALGORITHMS.forEach((algo) => {
    ...
    if (contains(fullText, "linked list")) {
...
function displayResult(result) {
  ...
  const badgeText = result.visualizerReady
    ? "✓ Visualizer Ready"
    : "Open Dashboard";

  const badgeClass = result.visualizerReady
    ? "ready-badge"
